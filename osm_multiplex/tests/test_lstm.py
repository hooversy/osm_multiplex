...
class TestWeekAnomalyDetect:
    def test_week_anomaly_detect(self):
        dataframes = {}
        ...
        anomalydetect = lstm.anomalydetect()
        test = anomalydetect.week_anomaly_detect(dataframes)
...
class TestRollingDataframes:
    def test_roll(self):
        data_list = [['2018-02-22 20:00:00', 44.44, 55.55, 3, 7],
                     ...
                     ['2018-02-24 03:30:00', 44.44, 55.55, 7, 8],
                     ['2018-02-24 04:00:00', 44.44, 55.55, 6, 5],
                     ['2018-02-24 04:30:00', 44.44, 55.55, 9, 8],
                     ['2018-02-24 05:00:00', 44.44, 55.55, 2, 2],
                     ['2018-02-24 05:30:00', 44.44, 55.55, 8, 8],
                     ['2018-02-24 06:00:00', 44.44, 55.55, 6, 5],
                     ['2018-02-24 06:30:00', 44.44, 55.55, 7, 8]]
        data = pd.DataFrame(data_list, columns=['time', 'lat', 'lon', 'occupancy1', 'occupancy2'])
        data['time'] =  pd.to_datetime(data['time'])
        data_multi = data.set_index(['time', 'lat', 'lon'])

        sampler = lstm.datasamples()
        test_rolling = sampler.rolling_sample(data_multi, length = 3)
